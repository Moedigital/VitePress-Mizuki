  ...
  <v-app id="inspire">
    ...
    <v-main class="bg-grey-lighten-3 card-with-background"
            ...
            <v-card rounded="lg" elevation="2" min-width="30vh">
              ...
              <v-sheet rounded="lg" v-for="sideitem in sidebar" :key="sideitem.text || 'sp'">
                ...
                <template v-else>
                  ...
                  <v-list rounded="lg">
                    <v-list-item
                        v-for="items in sidebar.filter(item => item.link && !item.items)"
                        :key="items.text"
                        :title="items.text"
                        :href="items.link"
                    ></v-list-item>
      ...
      <footer align="center">
        ...
        <v-btn :href="define_moecode_url" target="_blank" variant="text" v-if="footer_lib.moecode">萌ICP备 {{ footer_lib.moecode.match(/\d+/).toString() }} 号</v-btn>
        <p>由<a href="https://vitepress.dev" target="_blank" class="moedesign_a_element">VitePress</a>和可爱的<a href="https://mizukitheme.moedigital.org" target="_blank" class="moedesign_a_element">VitePress Mizuki</a>主题强力驱动</p>
        <br>
      </footer>
    </v-main>
  </v-app>
